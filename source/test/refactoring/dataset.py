...
if __name__ == '__main__':
    ...
    # shuffle
    data, target = corresponding_shuffle(iris.data, iris.target)

    # split data
    N_train = np.floor(n * 0.8).astype(np.int32)
    ...
    _datasets.train = Dataset(x_train, y_train)
    print("CREATING TEST DATASET...")
    _datasets.test = Dataset(x_test, y_test)
    print("CREATING VALIDATION DATASET...")
    _datasets.validation = Dataset(x_validation, y_validation)

    # save as a pickle file
    save_as_pickle('iris.pkl', _datasets)
